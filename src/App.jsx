...
// Version sans Firebase - Stockage en mémoire
const useAuth = () => {
...
export default function App() {
  ...
  const [requests, setRequests] = useState([
    { id: 1, user: 'Alex', type: 'Jouer', message: 'Qui veut jouer à Valorant ce soir ? 🎮', status: 'pending', responses: [{ user: 'Marie', response: 'yes' }] }
  ]);
  const [newRequestType, setNewRequestType] = useState('');
  const [newRequestMessage, setNewRequestMessage] = useState('');

  const [events, setEvents] = useState([
    { id: 1, title: '🎮 Soirée Gaming', date: '2025-10-25', time: '20:00', attendees: ['Alex', 'Marie'] }
  ]);
  ...
  const requestTypes = ['🎮 Jouer', '🍕 Manger', '🎬 Ciné', '⚽ Sport', '🎉 Sortir', '💪 Sport', '❓ Autre'];
  ...
  const addRequest = () => {
    ...
    setRequests([{ id: Date.now(), user: user?.username || 'Anonyme', type: newRequestType, message: newRequestMessage, status: 'pending', responses: [] }, ...requests]);
    setNewRequestType('');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-pink-50 to-orange-50">
      ...
      {/* Admin Panel avec animation */}
      {showAdminPanel && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fadeIn">
          <div className="bg-white rounded-3xl p-6 w-full max-w-md shadow-2xl transform scale-100 animate-slideUp">
            <h3 className="text-2xl font-black mb-4 bg-gradient-to-r from-purple-600 to-pink-500 bg-clip-text text-transparent">⚙️ Panel Admin</h3>
            {!isAdmin ? (
              <div className="space-y-4">
                <input
                  type="password"
      ...
      <main className="max-w-4xl mx-auto p-4 pb-20">
        {activeTab === 'requests' && (
          <div className="space-y-4 animate-fadeIn">
            {/* Ajouter une demande */}
            <div className="bg-white/90 backdrop-blur-xl rounded-3xl p-6 shadow-xl border border-white/20 hover:shadow-2xl transition-all duration-300">
          ...
          <div className="space-y-4 animate-fadeIn">
            <div className="bg-white/90 backdrop-blur-xl rounded-3xl p-6 shadow-xl border border-white/20">
              ...
              <div className="space-y-3">
                ...
                <button
                  onClick={addEvent}
                  className="w-full bg-gradient-to-r from-purple-600 to-pink-500 text-white py-3 rounded-xl font-bold hover:shadow-2xl transition-all transform hover:scale-105 flex items-center justify-center gap-2"
                >
                  <Sparkles className="w-5 h-5" />
                  Créer l'événement
                </button>
            ...
            {events.map((event, idx) => (
              <div key={event.id} className="bg-white/90 backdrop-blur-xl rounded-3xl p-6 shadow-xl border border-white/20 hover:shadow-2xl transition-all transform hover:-translate-y-1">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                ...
                {event.attendees?.length > 0 && (
                  <div className="mt-4 p-4 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <p className="font-bold text-green-700">{event.attendees.length} participant(s)</p>
                    </div>
                ...
                {(!event.attendees || !event.attendees.includes(user?.username)) && (
                  <button
                    ...
                    className="w-full mt-4 bg-gradient-to-r from-blue-500 to-purple-500 text-white py-3 rounded-xl font-bold hover:shadow-xl transition-all transform hover:scale-105"
                  >
                    🎉 Je participe !
                  </button>
                )}
        ...
        {activeTab === 'notifications' && (
          <div className="space-y-3 animate-fadeIn">
            {notifications.map((notif, idx) => (
              <div key={notif.id} className="bg-white/90 backdrop-blur-xl rounded-3xl p-4 shadow-xl border border-white/20 hover:shadow-2xl transition-all flex items-start gap-4 transform hover:-translate-y-1">
                <div className="w-12 h-12 bg-gradient-to-br from-purple-500 to-pink-500 rounded-2xl flex items-center justify-center flex-shrink-0 animate-pulse">
